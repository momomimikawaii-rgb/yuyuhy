...
{id:"auto",label:"おまかせ",color:"linear-gradient(135deg,#fce7f3,#ddd6fe,#bae6fd)",prompt:"世界観と服の基本方針に合わせて、一番可愛く見える服セットの色合いへ自動調整してください。"},
...
const HERO = "/top.png";
const UPDATED_AT = "2026/05/28（水） 11:55頃";
...
const animalWear = opt([
["auto","おまかせ（きぐるみ）","一緒にいる動物とおそろいに見える可愛いきぐるみ姿。動物の色・柄・耳・羽・模様・花冠などを合わせ、双子・親子・仲間のように見せる。",{kigurumi:true}],
["keep","なし（元写真のまま）","元写真のまま。"]
]);
...
const colors = opt([["auto","おまかせ","世界観と服の基本方針に合わせて、一番可愛く見える服セットの色合いへ自動調整してください。"],...outfitColorChoices.filter(c=>c.id!=="auto").map(c=>[c.id,c.label,c.prompt,{color:c.color}])]);
...
const [tpl,setTpl]=useState({travel:"mykonos",summer:"beach",vehicle:"sl",movie:"ship",animal:"friend",panel:"info"});
  ...
  const [cordoba,setCordoba]=useState("yuyu"), [gender,setGender]=useState("feminine"), [taste,setTaste]=useState("frill"), [outfit,setOutfit]=useState("auto"), [season,setSeason]=useState("auto"), [selectedOutfitColors,setSelectedOutfitColors]=useState(["auto"]), [customOutfit,setCustomOutfit]=useState("");
  const toggleOutfitColor=(id)=>{
    setSelectedOutfitColors(prev=>{
      if(id==="auto") return ["auto"];
      const withoutAuto = prev.filter(v=>v!=="auto");
      if(withoutAuto.includes(id)){
        const next = withoutAuto.filter(v=>v!==id);
        return next.length ? next : ["auto"];
      }
      if(withoutAuto.length>=3) return withoutAuto;
      return [...withoutAuto,id];
    });
  };
  const selectedOutfitColorPrompt = selectedOutfitColors.includes("auto")
    ? (cat==="animal"
      ? "一緒にいる動物やきぐるみの色・柄・世界観に合わせて、服セットの色合いを一番自然で可愛く見える配色におまかせで調整してください。"
      : by(colors,"auto").prompt)
    : selectedOutfitColors
        .map(id=>outfitColorChoices.find(c=>c.id===id)?.prompt)
        .filter(Boolean)
...
const [head,setHead]=useState("auto"), [customHead,setCustomHead]=useState("");
  ...
  const isSummerLocation = cat==="summer" || (cat==="travel" && ["mykonos"].includes(template.id));
  const isExtremeColdLocation = cat==="travel" && ["aurora","snowfield","iceworld"].includes(template.id);
  const isFourSeasonLocation = cat==="travel" && !isSummerLocation && !isExtremeColdLocation;
const isPanel=cat==="panel", isInfo=isPanel&&template.id==="info", isCalendar=isPanel&&template.id==="calendar";
  ...
  const prompt=useMemo(()=>{
    ...
    p.push(`【世界観・背景】\n${world}\n\n【背景と服の季節連動】\n${isSummerLocation ? "夏として扱い、背景は明るい夏・海・リゾート感にし、服も夏服に合わせてください。" : isExtremeColdLocation ? "酷寒として扱い、背景は雪・氷・オーロラの極寒世界にし、服ももこもこ防寒着寄りにしてください。" : season==="auto" ? "選んだ場所に合う季節、または服欄で選んだ季節を背景にも自然に反映してください。" : `${by(seasonOptions,season).label}として扱い、背景と服をその季節に合わせてください。`}${cat==="travel" ? `\n\n【世界旅行系 共通演出】
...
if(cat==="summer" && template.id==="fireworks" && outfit.startsWith("yukata")) p.push("【服セットの色合い】\n浴衣で選んだ色を最優先してください。追加の服色指定で浴衣の色を上書きしないでください。");
      else p.push(`【服セットの色合い】\n${customColor||selectedOutfitColorPrompt}`);
    }
    ...
    return p.join("\n\n");
  },[cat,template,customPlace,customVehicle,cordoba,fwPlace,title,gender,taste,outfit,season,isSummerLocation,isExtremeColdLocation,isFourSeasonLocation,outfitChoices,customOutfit,head,customHead,shoe,customShoe,acc,accMode,customAcc,color,customColor,selectedOutfitColorPrompt,customGimmick,currentUnder,summerAct,animal,animalColor,customAnimal,isInfo,isCalendar,infoStyle,panelText,year,month,paperId,dir,halloween,vibe,light,ratioPrompt,headChoices]);
  ...
  return <main className="page"><div className="blob blob-pink"/><div className="blob blob-violet"/><div className="blob blob-blue"/><div className="dots"/>
    <div className="container">
      <header className="hero"><div className="badge"><Sparkles size={18}/>Yuyu Mama Dream Prompt Studio</div><h1>ゆゆママの夢プロンプト工房（汎用版）</h1><p className="subtitle">うちの子を主役に、世界旅行・夏の海・乗り物・映画ポスター・うちの子パネルを作る工房です。</p><p className="selected">更新日時：2026/05/28（水） 11:55頃</p><div className="hero-image"><img src={HERO} alt="top"/></div><a className="sister-link" href={SISTER} target="_blank" rel="noreferrer"><LinkIcon size={16}/>姉妹サイト：ゆゆ姫の夢かわプロンプト工房はこちら</a></header>
      ...
      <div className="grid"><section className="left">
        ...
        {cat!=="movie"&&!isPanel&&<Section title="服"><p className="selected">服指定あり・おまかせの場合は、元写真の服・帽子・髪飾り・ハーネスなどを引き継がず、今回選んだ衣装だけで作ります。</p>
          ...
          <div className="subhead"><label>服</label><button type="button" className="outline-button mini" onClick={()=>{setCustomOutfit("");setOutfit("auto");setTaste("auto")}}>リセット</button></div>
          <div className="chips">{outfitChoices.filter(o=>{ if(isSummerLocation && o.id==="winterWear") return false; if(isFourSeasonLocation && ["swim","yukata","winterWear"].includes(o.id)) return false; if(isExtremeColdLocation) return ["keep","auto"].includes(o.id); return true; }).map(o=>o.id==="auto"&&cat!=="animal"
...
<input value={customOutfit} onChange={e=>setCustomOutfit(e.target.value)} placeholder="例：水色チェックのフリルワンピース"/>
          <>
            ...
            {!(cat==="summer"&&template.id==="fireworks"&&outfit.startsWith("yukata"))&&<><div className="subhead"><h2>服セットの色合い</h2><button type="button" className="outline-button mini" onClick={()=>{setCustomColor("");setColor("auto");setSelectedOutfitColors(["auto"])}}>リセット</button></div><div className="color-chip-wrap">{outfitColorChoices.map(c=><button
  type="button"
  key={c.id}
  title={c.label}
  className={`color-chip ${selectedOutfitColors.includes(c.id)?"active":""} ${c.id==="auto"?"color-chip-auto":""}`}
  style={{background:c.color}}
  onClick={()=>toggleOutfitColor(c.id)}
><span>{c.label}</span></button>)}</div><p className="selected">おまかせ、または最大3色まで選択可能</p><input value={customColor} onChange={e=>setCustomColor(e.target.value)} placeholder="色合い自由記入"/></>}
          </></Section>}
