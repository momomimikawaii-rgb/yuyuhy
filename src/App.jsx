...
const HERO = "/top.png";
const UPDATED_AT = "2026/05/27（水） 15:35頃";
...
const travelRule = `旅行カテゴリは「夢の観光ポスター構図」です。場所らしさが分かる背景量と、ペットの顔が見える距離を両立してください。一本道・行き止まり・ただの壁ドン構図は禁止。曲がる道、上へ続く階段、踊り場、奥へ続く小道、高低差、建物や寺院、海や空などを入れ、「この先へ行きたくなる」奥行きのある構図にしてください。近すぎてただの壁にならず、遠すぎてペットが豆粒にならないようにしてください。ペットだけのアップではなく、背景の観光地も主役級に見せます。現実の建物配置は完全再現不要です。`;
...
ペットは「その世界を旅する主人公」のように、観光ポスターの中心で魅力的に見せてください。\n世界旅行では背景の観光地も主役級に見せてください。ペットを大きくしすぎず、旅行写真・観光ポスターとして街並み・海・建物・空・奥行きが十分に見える構図にしてください。縦長画像では、ペットの高さは画像全体の35〜45％程度を目安にしてください。カメラは少し引き気味にし、ペットだけのアップ写真にしないでください。階段・道・水面・街並み・建物・空などの奥行きが広く見える構図にしてください。背景の情報量をしっかり残し、観光地の空気感も主役級にしてください。ただし豆粒のように小さくしすぎず、顔と服はちゃんと見えるサイズにしてください。横長画像でもペットが画面を占領しないようにし、顔は見えるけれど背景の名所が広く見えるバランスにしてください。ペットの旅行ポートレートではなく、ペットが旅する夢の観光ポスターとして作成してください。` : ""}`);
  ...
  return <main className="page"><div className="blob blob-pink"/><div className="blob blob-violet"/><div className="blob blob-blue"/><div className="dots"/>
    <div className="container">
      <header className="hero"><div className="badge"><Sparkles size={18}/>Yuyu Mama Dream Prompt Studio</div><h1>ゆゆママの夢プロンプト工房（汎用版）</h1><p className="subtitle">うちの子を主役に、世界旅行・夏の海・乗り物・映画ポスター・うちの子パネルを作る工房です。</p><p className="selected">更新日時：2026/05/27（水） 15:20頃</p><div className="hero-image"><img src={HERO} alt="top"/></div><a className="sister-link" href={SISTER} target="_blank" rel="noreferrer"><LinkIcon size={16}/>姉妹サイト：ゆゆ姫の夢かわプロンプト工房はこちら</a></header>
